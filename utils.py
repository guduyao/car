...
def bbox2loc(src_bbox, dst_bbox):
    ...
    # 求出最小的正数,将height,width与其比较保证全部是非负
...
def loc2bbox(src_bbox, loc):
    ...
    src_width = src_bbox[:, 2] - src_bbox[:, 0]   # ymax-ymin
    src_height = src_bbox[:, 3] - src_bbox[:, 1]  # xmax-xmin
    src_ctr_x = src_bbox[:, 0] + 0.5 * src_width  # y0+0.5h
    src_ctr_y = src_bbox[:, 1] + 0.5 * src_height # x0+0.5w,计算出中心点坐标
...
def bbox_iou(bbox_a, bbox_b):
    ...
    area_a = np.prod(bbox_a[:, 2:] - bbox_a[:, :2], axis=1) # 计算bboxa的面积
    area_b = np.prod(bbox_b[:, 2:] - bbox_b[:, :2], axis=1) # 计算bboxb的面积
