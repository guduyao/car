"""
    开发人：林培健 201806130001
    开发时间：2020年12月20号
"""


from transportation.nets.frcnn import FasterRCNN
from transportation.nets.frcnn_training import Generator
from torch.autograd import Variable
from transportation.trainer import FasterRCNNTrainer
import time
import numpy as np
import torch
...
def fit_ont_epoch(net, epoch, epoch_size, epoch_size_val, gen, genval, Epoch):
    ...
    for iteration, batch in enumerate(gen):
        ...
        print('iter:' + str(iteration) + '/' + str(
            epoch_size) + ' || total_loss: %.4f|| rpn_loc_loss: %.4f || rpn_cls_loss: %.4f || roi_loc_loss: %.4f || roi_cls_loss: %.4f || %.4fs/step' \
              % (total_loss / (iteration + 1), rpn_loc_loss / (iteration + 1), rpn_cls_loss / (iteration + 1),
                 roi_loc_loss / (iteration + 1), roi_cls_loss / (iteration + 1), waste_time))
    ...
    torch.save(model.state_dict(), 'E:/pycharm/car/transportation/logs/Epoch%d-Total_Loss%.4f-Val_Loss%.4f.pth' % (
...
if __name__ == "__main__":
    ...
    pretrained_dict = torch.load("E:/pycharm/car/transportation/model_data/voc_weights_resnet.pth", map_location=device)
